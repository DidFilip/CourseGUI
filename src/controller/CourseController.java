...
public class CourseController {
    ...
    @FXML
    private TextField tr_course_name;
    ...
    void addCourse(ActionEvent event) {
        //walidacja danych
        if(!tr_course_name.getText().equals("") && !tf_course_category.getText().equals("") && dp_course_date.getValue() !=null) {

            // pobieranie wartosci z kontrolki TextField
            String course_names = tr_course_name.getText();
            String course_categories = tf_course_category.getText();
            LocalDate course_date = dp_course_date.getValue();
            String course_trainers = tf_course_trainer.getText();
            // utworzenie obiektu klasy Course
            Course c = new Course(++id, course_names, course_categories, course_date, course_trainers);
            //wprowadzenie obiektu klasy Course do listy kursów
            courses.add(c);
            // odświeżenie TableView
            insertCoursesIntoTableViev();
            // czyszczenie pól po dodaniu kursu
            tr_course_name.clear();
    ...
    void getCourse(ActionEvent event) throws IOException {
        ...
        coursesStage.setTitle("Wybrany kurs");
        coursesStage.setScene(new Scene(root));
        coursesStage.show();


    }
    //----------metoda do wprowadzanie zawartosci listy kursów do tabelki --------
    private void insertCoursesIntoTableViev(){
        ...
        // wprowadzenie danych do tabeli z list ObservableList
        tbl_course.setItems(courses);
    }



    //        metoda która urachamia sie jako pierwsza
